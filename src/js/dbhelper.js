...
class DBHelper {
  ...
  static fetchRestaurants(callback) {
    ...
    /* if server fails to serve the data, use the cache data */
    xhr.onerror = () => {
        callback(error, null);
    };
    ...
    static fetchReviewsById(id, callback){
        ...
        xhr.onload = () => {
            if (xhr.status === 200) {
                ...
                console.log("Review From fetchRestaurant", reviews);
  ...
  static mapMarkerForRestaurant(restaurant, map) {
   ...
   // make restaurant favourite

  static restaurantFavouriteHandler(restaurant_id, favVal,callback) {
      
      let xhr = new XMLHttpRequest();
        xhr.open('PUT', 'http://localhost:1337/restaurants/'+restaurant_id+'/?is_favorite='+favVal, true);
